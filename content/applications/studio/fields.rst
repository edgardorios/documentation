...
The :guilabel:`Date Range` field is used to display a period of time defined by a start date and an
...
:guilabel:`Date Range` fields require another field to be selected as the start date *or* end date.
This other field can be an existing :ref:`Date field <studio/fields/simple-fields-date>` or
:guilabel:`Date & Time` field, or one created specifically for this purpose.

To add a date range:

#. If no existing field can be used as the underlying start/end date field, add a new
   :guilabel:`Date` or :guilabel:`Date & Time` field. If the date range:
   ...
   .. tip::
      To avoid showing the same information twice, make this field invisble by enabling
      :guilabel:`Invisible` or remove it from the view by clicking :guilabel:`Remove from view`.
...
The :guilabel:`Remaining Days` field displays the remaining number of days before the selected date
...
- :guilabel:`Invisible`: Enable this property when it is not necessary for users to view a field on
  ...
  .. note::
     The :guilabel:`Invisible` attribute also applies inside Studio. To view hidden fields in
     Studio, click on a view's :guilabel:`View` tab and enable :guilabel:`Show Invisible
     Elements`.
...
.. note::
   You can choose to enable :guilabel:`Invisble`, :guilabel:`Required` and :guilabel:`Readonly`
   for specific records only by clicking on :guilabel:`Conditional` and creating a filter.

- :guilabel:`Label`: the field's name on the UI.
...
- :guilabel:`Help Tooltip`: To explain the purpose of a field, add a description. The text is
  displayed inside a tooltip box when hovering with your mouse over the field's label.
...
- :guilabel:`Time interval`: controls the minute intervals shown in the time selector.
- :guilabel:`Warning for future dates`: displays a warning icon if a future date is chosen.
- :guilabel:`Show time`: On a read-only field, disabling this property means only the date is shown.
   This can keep a list view less cluttered, for example.
- :guilabel:`Earliest accepted date` and :guilabel:`Latest accepted date`: restricts the dates that
  can be selected in the date picker.
